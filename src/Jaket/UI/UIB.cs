...
public class UIB
{
    ...
    public static Button TeamButton(Team team, Transform parent, Rect r, Action clicked = null)
    {
        ...
        if (team == Team.V1)   Text("V1",  img.transform, r.Text, YAJF_yellow);
        if (team == Team.V2)   Text("V2",  img.transform, r.Text, YAJF_yellow);
    ...
    {
        var img = Image(name, parent, Btn(0f), YAJF_discord);
        Text(name, img.transform, Huge, size: 240).transform.localScale /= 10f;
        return Component<Button>(img.gameObject, button =>
        {
            button.targetGraphic = img;
            button.onClick.AddListener(() => Application.OpenURL("https://discord.gg/ANMa6psnwh"));
        });
    }
